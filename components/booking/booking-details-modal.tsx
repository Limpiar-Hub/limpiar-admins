    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px] p-0">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-semibold">{isRequest ? "Booking Request" : "Booking Details"}</h2>
                ...
                <div className="space-y-4">
                  {booking.timeline.map((event, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <div className="w-12 text-xs text-gray-500">{event.time}</div>
                      <div className="flex-1">
                        <div className="text-sm">
                          {event.event}{" "}
                          {event.assignedBusiness && (
                            <span className="text-blue-600 font-medium">{event.assignedBusiness}</span>
                          )}
                        </div>
        ...
        {showActionButtons && (
          <div className="flex justify-end gap-3 p-6 border-t">
            {isRequest ? (
              ...
              <Button className="bg-[#0082ed] hover:bg-[#0082ed]/90" onClick={onAssignBusiness}>
